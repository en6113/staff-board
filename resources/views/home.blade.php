<x-app-layout>
    ...
    <div class="py-12 bg-gray-100 min-h-screen">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            {{-- 画面が広いとき（lg以上）だけ 12分割グリッドの2カラムにする --}}
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">

                {{-- ===================================================== --}}
                {{-- 左カラム：チャット --}}
                {{-- スマホでは下に回すため order-last / lg:order-first --}}
                {{-- ===================================================== --}}
                <aside class="lg:col-span-4 order-last lg:order-first lg:sticky lg:top-8">
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                        <div class="flex justify-between items-center mb-4 border-b pb-2">
                            ...
                            <a href="{{ route('posts.index') }}"
                                class="text-sm font-medium text-green-600 hover:text-green-800 hover:underline flex items-center gap-1">
                        ...
                        @else
                            <ul class="divide-y divide-gray-200">
                                @foreach($posts as $post)
                                    <li
                                        class="py-3 flex justify-between items-center hover:bg-gray-50 px-2 rounded-lg transition">
                                        <div class="flex items-center space-x-3 min-w-0 flex-1">
                                            {{-- 区分（掲示 or 回覧） --}}
                                            <span
                                            ...
                                            <div class="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between gap-4">
                                                <a href="{{ route('posts.show', $post->id) }}"
                                                    class="block text-sm font-medium text-gray-900 hover:text-green-600 truncate">
                                                    {{ $post->title }}
                                                </a>
                                                <span class="block text-gray-400 shrink-0 mt-1 sm:mt-0">
